...
class KeyFrameDatabase(object):
    def __init__(self, config, H, W, num_kf, num_rays_to_save, device) -> None:
        ...
        self.keyframes = {}
        self.device = device
        self.rays = torch.zeros((num_kf, num_rays_to_save, 8))  #scan:8  replice:11 tum:7
    ...
    def get_keyframe(self, keyframe_id):
        ...
        Parameters:
            keyframe_id (int): 关键帧标识符

        Returns:
            dict or None: 包含关键帧信息的字典，如果未找到则返回 None
        """
    ...
    def add_keyframe(self, batch,train, filter_depth=False):
        '''
        ...
        if train:
            rays = torch.cat([batch['direction'], batch['rgb'], batch['depth'][..., None], batch['semantic'][..., None]], dim=-1)
    ...
    def sample_global_rays(self, bs):
        ...
        sample_rays = self.rays[:num_kf].reshape(-1,8)[idxs]   #scan:8  replice-tum:7
    ...
    @torch.no_grad()
    def sample_overlap_keyframe(self, batch, frame_id, est_c2w_list, k_frame, dataset,n_samples=16, n_pixel=100 ):
        '''
        ...
        for i, frame in enumerate(self.frame_ids):
            frame = int(frame.item())
            c2w = est_c2w_list[frame].cpu().numpy()
            ...
            percent_inside = mask.sum() / mask.size
            key_frame_list.append(
                {'id': frame, 'percent_inside': percent_inside, 'sample_id':i})



        key_frame_list = sorted(key_frame_list, key=lambda i: i['percent_inside'], reverse=True)
        selected_keyframe_list = [dic['sample_id'] for dic in key_frame_list if dic['percent_inside'] > 0.50]
