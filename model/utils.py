...
def compute_cross_entropy_loss(sem_logits_coarse, sampled_gt_semantic):
    """
    Compute the cross entropy loss for semantic segmentation.

    Parameters:
    - sem_logits_coarse: Tensor, model output logits for each class [1954, 26]
    - sampled_gt_semantic: Tensor, ground truth semantic labels [1954]

    Returns:
    - loss: Tensor, scalar value of the computed cross entropy loss
    """
    # print("sem_logits_coarse is ",sem_logits_coarse.shape)
    # print("sampled_gt_semantic is ", sampled_gt_semantic)
    # 确保标签是长整型，因为CrossEntropyLoss期望标签是整数
    sampled_gt_semantic = sampled_gt_semantic.long()

    # 创建CrossEntropyLoss实例，可以选择设置 ignore_index 参数
    criterion = nn.CrossEntropyLoss(ignore_index=-1)

    # 直接计算损失，因为 sampled_gt_semantic 已经是 [1954] 维度，与 sem_logits_coarse 的第一维匹配
    loss = criterion(sem_logits_coarse, sampled_gt_semantic)
