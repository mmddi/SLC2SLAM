...
class SemanticNet(nn.Module):
    def __init__(self,config, input_ch,output_ch, geo_feat_dim=15, hidden_dim_semantic=64, num_layers_semantic=4):
    ...
    def get_model(self, tcnn_network=False):
        if tcnn_network:
           ...
           return tcnn.Network(
                ...
                network_config={
                    ...
                    "n_hidden_layers":self.num_layers_semantic - 1,
                }
                # dtype=torch.float
            )
        sem_net = []
        for l in range(self.num_layers_semantic):
            ...
            if l != self.num_layers_semantic - 1:
                # sem_net.append(nn.BatchNorm1d(out_dim))
                sem_net.append(nn.ReLU(inplace=True))
                # sem_net.append(nn.Dropout(0.5))  # Dropout with 50% rate


        return nn.Sequential(*sem_net)
...
class ColorSDFSemNet(nn.Module):
    '''
       No color grid
       '''

    def __init__(self, config,output_ch_semantic, input_ch=3, input_ch_pos=12):
        super(ColorSDFSemNet, self).__init__()
        self.config = config
        self.color_net = ColorNet(config,
                                  input_ch=input_ch_pos,
                                  geo_feat_dim=config['decoder']['geo_feat_dim'],
        ...
        self.sdf_net = SDFNet(config,
                              ...
                              hidden_dim=config['decoder']['hidden_dim'],
                              num_layers=config['decoder']['num_layers'])
        self.semantic_net = SemanticNet(config,input_ch=input_ch_pos,
                                        output_ch=output_ch_semantic,
                                        geo_feat_dim=config['decoder']['geo_feat_dim'],
                                        hidden_dim_semantic=config['decoder']['hidden_dim_semantic'],
                                        num_layers_semantic=config['decoder']['num_layers_semantic'])

    def forward(self, embed, embed_pos,train_semantic=False):
        ...
        if embed_pos is not None:
            ...
            if train_semantic:
                ...
                sem = self.semantic_net(torch.cat([embed_pos, geo_feat], dim=-1))
                return torch.cat([rgb, sdf, sem], -1)
            else:
                return torch.cat([rgb, sdf], -1)
        else:
            rgb = self.color_net(torch.cat([geo_feat], dim=-1))
            if train_semantic:
                sem = self.semantic_net(torch.cat([geo_feat], dim=-1))
                return torch.cat([rgb, sdf, sem], -1)
            else:
                return torch.cat([rgb, sdf], -1)
