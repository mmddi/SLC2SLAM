...
def render_depth_maps(mesh, poses, K, H, W, near=0.01, far=5.0):
    ...
    poses = [np.array(pose).astype(np.float32) for pose in poses]
...
def cull_from_one_pose(points, pose, K, H, W, remove_occlusion=True, rendered_depth=None, eps=0.03):
    ...
    # print("Type of c2w:", type(c2w))
    # print("Data type of c2w elements:", c2w.dtype)
...
if __name__ == "__main__":
    ...
    if args.gt_pose:
        ...
        c2w_list = [pose.cpu().numpy().astype(np.float32) for pose in dataset.poses[::args.skip]]
        # c2w_list = [pose.numpy().astype(np.float32) for pose in dataset.poses[::args.skip]]
    else:
