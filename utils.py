...
def extract_mesh(query_fn, config, bounding_box, marching_cube_bound=None, color_func = None, voxel_size=None, resolution=None, isolevel=0.0, scene_name='', mesh_savepath=''):
    ...
    query_pts = torch.stack(torch.meshgrid(tx, ty, tz, indexing='ij'), -1).to(torch.float32)#调用 getVoxels 函数来计算在给定分辨率和体素大小下的体素坐标。
    ...
    fn = get_batch_query_fn(query_fn, device=bounding_box.device)  #获取批量查询函数，用于并行查询网络。  self.model.query_sdf,
    ...
    # vertices, triangles = marching_cubes(raw.squeeze(), isolevel, truncation=3.0)
